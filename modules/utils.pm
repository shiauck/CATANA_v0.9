...
sub dump_log($$$$$$$$$$$) {
   my($pre_string, $mid_string, $suf_string, $seq_arr, $tx_1_arr, $tx_2_arr, $tx_1, $tx_2, $gene_id, $strand, $log_f) = @_;
   my(@tmp_arr, $pattern);
   ...
   draw_ae($gene_id, $tx_1, $tx_2, "SE", \@tmp_arr, $seq_arr, $strand, $log_f) if $#tmp_arr > -1;

   my($mse_arr, $mxe_arr) = detect_ae::detect_mse_mxe($pre_string, $mid_string, $suf_string,
                                           $tx_1_arr, $tx_2_arr, $tx_1, $tx_2);
   ++$ae_type_counter if $#$mse_arr > -1;
   draw_ae($gene_id, $tx_1, $tx_2, "MSE", $mse_arr, $seq_arr, $strand, $log_f) if $#$mse_arr > -1;

   ++$ae_type_counter if $#$mxe_arr > -1;
   draw_ae($gene_id, $tx_1, $tx_2, "MXE", $mxe_arr, $seq_arr, $strand, $log_f) if $#$mxe_arr > -1;

   @tmp_arr = detect_ae::detect_ri($pre_string,   $mid_string, $suf_string, $tx_1_arr, $tx_2_arr, $tx_1, $tx_2);
   ++$ae_type_counter if $#tmp_arr > -1;
   draw_ae($gene_id, $tx_1, $tx_2, "RI", \@tmp_arr, $seq_arr, $strand, $log_f) if $#tmp_arr > -1;

   @tmp_arr = detect_ae::detect_a5ss($pre_string, $mid_string, $suf_string, $tx_1_arr, $tx_2_arr, $tx_1, $tx_2);
   ++$ae_type_counter if $#tmp_arr > -1;
   $pattern = ($strand eq '+')?("A5SS"):("A3SS");
   draw_ae($gene_id, $tx_1, $tx_2, $pattern, \@tmp_arr, $seq_arr, $strand, $log_f) if $#tmp_arr > -1;
...
sub draw_ae($$$$$$$$) {
   my($gene_id, $tx_1, $tx_2, $as_type, $pos_arr, $seq_arr, $strand, $log_f) = @_;

   open(LOG, ">>$log_f");

   for(my $i = 0; $i <= $#$pos_arr; $i++)
   {
      print LOG "$gene_id\t$tx_1\t$tx_2\t$as_type\t";
